...
import { ComponentLoadingStore, getDefaultComponentLoadingState, } from './component-loading-store.service';
...
describe('LoadingStore', () => {
    ...
    beforeEach(() => {
        ...
        vi.spyOn(store, 'createEffect').mockImplementation((effect) => effect(action$));
    });

    const getDispatchAction = <ITEM, ERROR>({ actionName, storeState, }: {
        actionName: string;
    ...
    describe('loadingEffect', () => {
        ...
        it('should change state while effect is running', async () => {
            vi.useFakeTimers();
            const testEffect = store.loadingEffect('testEffect', () => cold('-a-#', { a: 'newValue' }, 500));
            ...
            expect(store.state$).toBeObservable(cold('ab-c', {
        ...
        it('should change state while effect is running in cache mode (with option: canCache)', async () => {
            vi.useFakeTimers();
            const testEffect = store.loadingEffect('testEffect', () => cold('-a-|', { a: 'newValue' }), { canCache: true });
            store.patchState({ item: 'oldValue', error: 404 });
            await vi.waitFor(() => {
                expect(mockStore.dispatch).toHaveBeenCalled();
            });
            ...
            expect(store.state$).toBeObservable(cold('ab', {
                a: getDefaultComponentLoadingState({
                    isLoading: true,
                    item: 'oldValue',
                    error: 404,
        ...
        it('should change state while effect is running in cache mode (with option: skipSamePendingActions)', async () => {
            vi.useFakeTimers();
            const testEffect = store.loadingEffect('testEffect', () => cold('-a-|', { a: 'newValue' }), { skipSamePendingActions: true });
            store.patchState({ item: 'oldValue', error: 404 });
            await vi.waitFor(() => {
                expect(mockStore.dispatch).toHaveBeenCalled();
            });
        ...
        it('should skip skip same actions for option skipSameActions', () => {
            ...
            }) => spyEffectRun(), { skipSameActions: true });
            store.patchState({ item: 'oldValue', error: 404 });
            mockStore.dispatch.mockClear();
        ...
        it('should skip skip same actions for option skipSameActions, without payload', () => {
            const spyEffectRun = vi.fn().mockReturnValue(of('newValue'));
            const testEffect = store.loadingEffect('testEffect', () => spyEffectRun(), { skipSameActions: true });
            store.patchState({ item: 'oldValue', error: 404 });
            mockStore.dispatch.mockClear();
        ...
        it('should replay effect', () => {
            ...
            const testEffect = store.loadingEffect('testEffect', () => of('newValue').pipe(tap(() => spyEffectRun())), { repeatActions: [customAction] });
